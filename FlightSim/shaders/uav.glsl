...
vec3 lightPos = vec3(-10.0f, 60.0f, 150.0f);
...
vec3 toLight = normalize(lightPos - v2f_Pos.xyz); 
	...
	vec3 specular = pow( max(0,dot(normal, halfVector)) , 150) * specularColor ;
	color = vec4(diffuse + specular + vec3(0.2), 1.0);

//	color = vec4(diffuse + specular + vec3(0.2), 1.0);
//	vec3 I = normalize(v2f_Pos.xyz-u_CameraPosition);
//	vec3 R = reflect(I, normalize(v2f_Normal));
//	color = vec4(texture(skybox, R).rgb,1.0)+vec4(0.2);
